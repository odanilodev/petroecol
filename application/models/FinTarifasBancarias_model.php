...
class FinTarifasBancarias_model extends CI_Model
{
    ...
    public function recebeNomeTarifa($nome_tarifa, $id)
    {
    ...
    public function inativaTarifaBancaria($id)
    {
        $this->db->where('id', $id);
        $this->db->where('id_empresa', $this->session->userdata('id_empresa'));
        $this->db->set('status', 0);
        $this->db->update('fin_tarifas_bancarias');
    ...
    {
        $this->db->where('id_tarifa_bancaria', $id);
        $this->db->where('id_empresa', $this->session->userdata('id_empresa'));
